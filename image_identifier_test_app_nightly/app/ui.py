...
from .config import APP_NAME, APP_VERSION, LOGO_PATH, MARKED_DIR, SNAPSHOT_DIR, REPORT_DIR, STORAGE_DIR, SETTINGS_PATH, WATCH_INBOX_DIR
...
class ImageIdentifierApp(ctk.CTk):
    def __init__(self):
        ...
        self.watch_folder_var = tk.StringVar(value=str(WATCH_INBOX_DIR.resolve()))
        self.watch_action_var = tk.StringVar(value="Ask Each Time")
    ...
    def _load_settings(self):
        try:
            if SETTINGS_PATH.exists():
                ...
                self.watch_folder_var.set(data.get("watch_folder", self.watch_folder_var.get()))
                self.watch_action_var.set(data.get("watch_action", self.watch_action_var.get()))
        except Exception:
            pass
    ...
    def _save_settings(self):
        try:
            ...
            SETTINGS_PATH.write_text(json.dumps({
                ...
                "watch_action": self.watch_action_var.get(),
    ...
    def _build_advanced(self):
        ...
        external_tab = tabs.add("External Workflow")
        ...
        self._build_external_workflow_tab(external_tab)

    def _build_external_workflow_tab(self, parent):
        shell = ctk.CTkFrame(parent, fg_color="transparent")
        shell.pack(fill="both", expand=True, padx=14, pady=14)
        ctk.CTkLabel(shell, text="External Workflow", font=("Segoe UI", 18, "bold"), text_color=TEXT).pack(anchor="w", pady=(0, 8))

        row1 = ctk.CTkFrame(shell, fg_color=PANEL_2, corner_radius=14)
        row1.pack(fill="x", pady=6)
        ctk.CTkLabel(row1, text="Watch Folder", text_color=MUTED, width=120).pack(side="left", padx=12, pady=12)
        ctk.CTkEntry(row1, textvariable=self.watch_folder_var, fg_color=PANEL, border_color=BLUE).pack(side="left", fill="x", expand=True, padx=8, pady=12)
        ctk.CTkButton(row1, text="Change", command=self.change_watch_folder, fg_color=BLUE, hover_color=HOVER_BLUE, width=90).pack(side="right", padx=8, pady=12)
        ctk.CTkButton(row1, text="Open", command=lambda: self._open_path(Path(self.watch_folder_var.get())), fg_color=PANEL, hover_color="#262b38", width=70).pack(side="right", padx=8, pady=12)

        row2 = ctk.CTkFrame(shell, fg_color=PANEL_2, corner_radius=14)
        row2.pack(fill="x", pady=6)
        ctk.CTkLabel(row2, text="New File Action", text_color=MUTED, width=120).pack(side="left", padx=12, pady=12)
        ctk.CTkOptionMenu(row2, values=["Ask Each Time", "Scan Only", "Mark Automatically"], variable=self.watch_action_var, command=lambda _v: self._save_settings(), fg_color=PANEL, button_color=BLUE, button_hover_color=HOVER_BLUE, dropdown_fg_color=PANEL_2, dropdown_hover_color=RED).pack(side="left", padx=8, pady=12)
        ctk.CTkButton(row2, text="Start Watching" if not self.watch_enabled_var.get() else "Stop Watching", command=self.toggle_watch_folder, fg_color=GREEN if not self.watch_enabled_var.get() else RED, hover_color=HOVER_GREEN if not self.watch_enabled_var.get() else HOVER_RED, width=150).pack(side="right", padx=12, pady=12)

        row3 = ctk.CTkFrame(shell, fg_color=PANEL_2, corner_radius=14)
        row3.pack(fill="x", pady=6)
        ctk.CTkButton(row3, text="Install / Refresh Companion Scripts", command=self.create_companion_scripts, fg_color=BLUE, hover_color=HOVER_BLUE).pack(side="left", padx=12, pady=12)
        ctk.CTkButton(row3, text="Open Scripts Folder", command=lambda: self._open_path(Path(__file__).resolve().parent.parent / "companion_scripts"), fg_color=PANEL, hover_color="#262b38").pack(side="left", padx=8, pady=12)

        self.watch_status_label = ctk.CTkLabel(shell, text=self._watch_status_text(), text_color=TEXT, justify="left", wraplength=760)
        self.watch_status_label.pack(anchor="w", padx=4, pady=(14, 0))
    ...
    def change_watch_folder(self):
        initial = Path(self.watch_folder_var.get()).expanduser()
        chosen = filedialog.askdirectory(title="Choose external workflow watch folder", initialdir=str(initial if initial.exists() else STORAGE_DIR))
        if chosen:
            self.watch_folder_var.set(chosen)
            Path(chosen).mkdir(parents=True, exist_ok=True)
            self._save_settings()
            self._log(f"Watch folder changed: {chosen}")
            self._render_mode()

    def _watch_status_text(self):
        state = "Active" if self.watch_enabled_var.get() else "Stopped"
        return f"Status: {state}\nFolder: {self.watch_folder_var.get()}\nAction: {self.watch_action_var.get()}"
        ...
        if self.watch_enabled_var.get():
            ...
            if self.watch_job:
                try:
                    ...
                    pass
            self.watch_job = None
            self._log("External watch folder stopped.")
        else:
            folder = Path(self.watch_folder_var.get()).expanduser()
            folder.mkdir(parents=True, exist_ok=True)
            self.watch_seen = {str(p): p.stat().st_mtime for p in folder.glob("*") if p.is_file()}
            self.watch_enabled_var.set(True)
            self._save_settings()
            self._log(f"External watch folder started: {folder}")
    ...
    def _poll_watch_folder(self):
        ...
        for path in sorted(folder.glob("*")):
            ...
            try:
                ...
                continue
            if self.watch_seen.get(key) == "processed":
                continue
            if key not in self.watch_seen:
                self.watch_seen[key] = stat.st_mtime
                continue
            if now - stat.st_mtime < 1.5:
                continue
            self.watch_seen[key] = "processed"
            self._handle_external_file(path)
        self.watch_job = self.after(1800, self._poll_watch_folder)

    def _handle_external_file(self, path: Path):
        self.selected_paths = [path]
        self.selected_card_indices = {0}
        self.last_action_summary.set(f"External file detected: {path.name}")
        self._log(f"External file detected: {path}")
        if hasattr(self, "preview_scroll"):
            self._display_submitted_images(self.selected_paths)
        action = self.watch_action_var.get()
        if action == "Scan Only":
            self.scan_selected()
        elif action == "Mark Automatically":
            self.mark_selected()
        else:
            self._show_app_confirm(
                "External Image Detected",
                f"{path.name} was added from the watch folder. Choose how to handle it.",
                [("Scan", self.scan_selected), ("Mark", self.mark_selected), ("Ignore", None)],
            )

    def create_companion_scripts(self):
        scripts_dir = Path(__file__).resolve().parent.parent / "companion_scripts"
        scripts_dir.mkdir(parents=True, exist_ok=True)
        wf = Path(self.watch_folder_var.get()).expanduser()
        watch_folder_js = str(wf).replace('\\', '/')
        jsx = (
            '/* Image Identifier Photoshop handoff script.\n'
            '   Use: File > Scripts > Browse..., run this script, then scan/mark from the app. */\n'
            '#target photoshop\n'
            'if (app.documents.length === 0) {\n'
            '    alert("No active Photoshop document.");\n'
            '} else {\n'
            f'    var outFolder = new Folder("{watch_folder_js}");\n'
            '    if (!outFolder.exists) outFolder.create();\n'
            '    var doc = app.activeDocument;\n'
            '    var safeName = doc.name.replace(/\\.[^\\.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, "_");\n'
            '    var outFile = new File(outFolder.fsName + "/" + safeName + "_identifier_handoff.png");\n'
            '    var opts = new PNGSaveOptions();\n'
            '    doc.saveAs(outFile, opts, true, Extension.LOWERCASE);\n'
            '    alert("Exported to Image Identifier watch folder:\\n" + outFile.fsName);\n'
            '}\n'
        )
        (scripts_dir / "Send_To_Image_Identifier_Photoshop.jsx").write_text(jsx, encoding="utf-8")
        lua = (
            '-- Lightroom Classic export helper concept for Image Identifier.\n'
            '-- Easiest setup: create an Export Preset with Export Location set to:\n'
            f'-- {wf}\n'
            '-- Then start Watch Folder in Image Identifier before exporting.\n'
            'return {\n'
            '  title = "Send to Image Identifier",\n'
            f'  watchFolder = [[{wf}]],\n'
            '  note = "Use this folder as the export destination for your Lightroom preset."\n'
            '}\n'
        )
        (scripts_dir / "Lightroom_Export_Preset_Helper.lua").write_text(lua, encoding="utf-8")
        guide = f"""Image Identifier External Workflow Scripts
...
3. Select Send_To_Image_Identifier_Photoshop.jsx.
4. The active document exports into the watch folder.

Lightroom Classic / Luminar Neo:
1. Create an export preset.
2. Set the export destination to the watch folder above.
3. Start Watch Folder inside Image Identifier.
4. Export selected image(s).

Recommended action mode:
- Ask Each Time while testing.
- Scan Only for intake review.
- Mark Automatically only when you trust the workflow.
"""
        (scripts_dir / "README_EXTERNAL_WORKFLOW.txt").write_text(guide, encoding="utf-8")
        self._save_settings()
        self._log(f"Companion scripts created: {scripts_dir}")
        self._show_app_confirm("Scripts Created", f"Companion scripts were saved to:\n{scripts_dir}", [("Ok", None), ("Open Folder", lambda: self._open_path(scripts_dir))])
